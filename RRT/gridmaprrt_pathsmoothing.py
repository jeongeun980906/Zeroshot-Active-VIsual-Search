...
def get_target_point(path, targetL,rrtplanner,verbose):
    ...
    try:
        ...
        print(i)
        get_target_point(path, targetL,rrtplanner,verbose)
    from_xz,to_xz = rstate[path[ti]],rstate[path[ti+1]]

    to_xz = np.array([to_xz[0] * partRatio + from_xz[0] * (1-partRatio),
    ...
    to_node, safty_flag = rrtplanner.steer_collision(from_node, to_node, extend_length=float("inf"),verbose=False, resolution= 5)
    ...
    # path = np.array([fr_idx,to_idx])
    
    # final_pos, safty_flag = rrtplanner.get_Navigation_success_flag(path,verbose)
...
def line_collision_check(first, second,rrtplanner,verbose):
    ...
    _, safty_flag = rrtplanner.steer_collision(from_node, to_node, extend_length=float("inf"),verbose=verbose, resolution=20)
...
def path_smoothing(rrtplanner, max_iter,verbose=False):
    ...
    for i in range(max_iter):
        ...
        if templabel:
            first,flag1 = get_target_point(path, pickPoints[0],rrtplanner,verbose=verbose)
            second,flag2 = get_target_point(path, pickPoints[1],rrtplanner,verbose=verbose)
            line_collision_check(first, second, rrtplanner,verbose=verbose)
...
#     path = np.array([first[1],second[1]])
    
#     final_pos, safty_flag = rrtplanner.get_Navigation_success_flag(path,verbose=verbose)
