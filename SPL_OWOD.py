...
from detector.thor import gather,detect_OWOD
...
# OWOD
from eval_ithor.owod import load_OWOD,gather_owod
...
def main(args):
    ...
    step = 5
    # random.seed(10)
    ST = score_storage(args,owod=True)
    if args.resume:
        ST.load_json()
    ...
    Load Detector
    '''
    predictor,unk_dist,known_dist = load_OWOD()
    ...
    for scene_name in scene_names:
        ...
        for query_object in tqdm(query_objects,desc=scene_name):
            ...
            while not total_success:
                # scanning
                frames,single_pos,gt_boxes,gt_vis,detected_landmarks = gather_owod(controller,[query_object['objectId']],opos,
                    predictor,unk_dist,known_dist,clip_matcher,
                    detection_labels,scene_memory)
                # print('gt_vis?',gt_vis)
                candidate_patches, candidate_map_points,sucesses = detect_OWOD(frames,single_pos,gt_boxes,controller,predictor,clip_matcher,d2w,unk_dist,known_dist)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--version',default = 5, type=int,help='version')
    ...
    parser.add_argument('--unct_ratio',type = float ,default=0.05,help='unct ratio')
    # CLIP setup
